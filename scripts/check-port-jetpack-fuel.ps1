...
try {
    ...
    Invoke-PortJUnitTests -GradleExecutable $gradle -ProjectDirectory $libraryRoot `
        ...
        -TestPattern 'com.gregtechceu.gtceu.common.item.armor.JetpackFuelSelectionPortTest'
} finally {
    $env:JAVA_HOME = $previousJavaHome
}

Write-Output 'These checks execute the production jetpack fuel-selection helper with the real FluidIngredient and NeoForge FluidStack types. They do not load PowerlessJetpack behavior, tank capabilities, recipe reload, or armor/player ticks.'
