...
try {
    ...
    Invoke-PortJUnitTests -GradleExecutable $gradle -ProjectDirectory $libraryRoot `
        -InitScript (Join-Path $PSScriptRoot 'port-enum-ordinal.init.gradle') `
        -TestPattern 'com.gregtechceu.gtceu.utils.EnumOrdinalPortTest'
} finally {
    $env:JAVA_HOME = $previousJavaHome
}

Write-Output 'These tests cover persisted enum ordinal selection/cycling; individual cover deserializers and paste interactions are not loaded.'
