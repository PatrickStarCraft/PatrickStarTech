...
try {
    ...
    Invoke-PortJUnitTests -GradleExecutable $gradle -ProjectDirectory $libraryRoot `
        -InitScript (Join-Path $PSScriptRoot 'port-overclocking.init.gradle') `
        -TestPattern 'com.gregtechceu.gtceu.api.recipe.OverclockingLogicPortTest'
} finally {
    $env:JAVA_HOME = $previousJavaHome
}

Write-Output 'These checks execute the production overclock algorithms only; machine parallel selection, recipe modification, and world-tick behavior are not loaded.'
