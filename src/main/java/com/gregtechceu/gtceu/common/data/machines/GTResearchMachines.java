...
public class GTResearchMachines {
    ...
    public static final MultiblockMachineDefinition RESEARCH_STATION = REGISTRATE
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .slice("XXX", "VAV", "XAX", "XSX", "XAX", "VAV", "XXX")
                    .slice("XXX", "XAX", "---", "---", "---", "XAX", "XXX")
                    .slice(" X ", "XAX", "---", "---", "---", "XAX", " X ")
                    .slice(" X ", "XAX", "-A-", "-H-", "-A-", "XAX", " X ")
                    .slice("   ", "XXX", "---", "---", "---", "XXX", "   ")
                    ...
                    .where('P', blocks(COMPUTER_CASING.get())
                            .and(abilities(PartAbility.INPUT_ENERGY).setMinGlobalLimited(1).setMaxGlobalLimited(2, 1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_RECEPTION).setExactLimit(1))
    ...
    public static final MultiblockMachineDefinition DATA_BANK = REGISTRATE.multiblock("data_bank", DataBankMachine::new)
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .slice("XDDDX", "XAAAX", "XDDDX")
                    .slice("XCCCX", "XCSCX", "XCCCX")
                    .where('S', controller(blocks(definition.getBlock())))
                    .where('X', blocks(COMPUTER_HEAT_VENT.get()))
                    .where('D', blocks(COMPUTER_CASING.get()).setMinGlobalLimited(3)
                            .and(abilities(PartAbility.DATA_ACCESS).setPreviewCount(3))
                            .and(abilities(PartAbility.OPTICAL_DATA_TRANSMISSION).setMinGlobalLimited(1, 1))
                            .and(abilities(PartAbility.OPTICAL_DATA_RECEPTION).setPreviewCount(1)))
    ...
    public static final MultiblockMachineDefinition NETWORK_SWITCH = REGISTRATE
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .where('X', blocks(COMPUTER_CASING.get()).setMinGlobalLimited(7)
                            .and(abilities(PartAbility.INPUT_ENERGY).setMinGlobalLimited(1).setMaxGlobalLimited(2, 1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_TRANSMISSION).setMinGlobalLimited(1, 1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_RECEPTION).setMinGlobalLimited(1, 2))
                            .and(autoAbilities(true, false, false)))
                    .build())
    ...
    public static final MultiblockMachineDefinition HIGH_PERFORMANCE_COMPUTING_ARRAY = REGISTRATE
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .where('C', blocks(COMPUTER_CASING.get()).setMinGlobalLimited(5)
                            .and(abilities(PartAbility.INPUT_ENERGY).setMinGlobalLimited(1).setMaxGlobalLimited(2, 1))
                            .and(abilities(PartAbility.IMPORT_FLUIDS).setMaxGlobalLimited(1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_TRANSMISSION).setExactLimit(1))
                            .and(autoAbilities(true, false, false)))
                    .build())
            .sidedWorkableCasingModel(GTCEu.id("block/casings/hpca/computer_casing"),
